...
@section('content')
    ...
    <div class="flex flex-col md:flex-row px-4 gap-2 justify-between">
        <div>
            <a href="/tagihan-blbi" class="btn btn-neutral btn-sm">Sebelumnya</a>
            <a href="/tagihan-blbi/{{ $tagihan->id }}/rekanan/create" class="btn btn-neutral btn-sm">Tambah</a>
        </div>
        <div>
        ...
        <x-rekanan>
            @foreach ($data as $item)
                <tr>
                    ...
                    <x-table.body.column class="border text-center">
                        <div class="join">
                            <a href="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $item->id }}/ppn"
                                class="btn btn-xs btn-outline btn-neutral join-item">PPN</a>
                            <a href="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $item->id }}/pph"
                                class="btn btn-xs btn-outline btn-neutral join-item">PPh</a>
                            <form action="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $item->id }}" method="post">
                                @csrf
                                @method('DELETE')
