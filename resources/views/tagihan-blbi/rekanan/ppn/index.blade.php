...
@section('content')
    ...
    <div class="flex flex-col md:flex-row px-4 gap-2 justify-between">
        <div>
            ...
            <a href="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $rekanan->id }}/ppn/create"
            ...
            @foreach ($data as $item)
                <tr>
                    ...
                    <x-table.body.column class="border">
                        <div class="join">
                            <a href="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $rekanan->id }}/ppn/{{ $item->id }}/edit"
                                class="btn btn-xs btn-outline btn-neutral join-item">edit</a>
                            <form
                                action="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $rekanan->id }}/ppn/{{ $item->id }}"
                                method="post">
                                @csrf
                                @method('DELETE')
