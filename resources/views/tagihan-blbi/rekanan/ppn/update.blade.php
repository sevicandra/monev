...
@section('content')
        <div class="bg-primary p-4">
            <h1 class="text-xl text-primary-content">Edit PPN</h1>
        </div>
        <div class="px-4">
            <form id="inputForm" action="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $rekanan->id }}/ppn/{{ $data->id }}" method="post" autocomplete="off">
                @csrf
                @method('PATCH')
                ...
                <div class="form-control w-full max-w-xs">
                    ...
                    <input type="date" name="tanggalfaktur"
                        ...
                        value="{{ $data->tanggalfaktur }}" placeholder="dd-mm-yyyy" />
                    <label class="label">
                        @error('tanggalfaktur')
                            <span class="label-text-alt text-red-500">
                                {{ $message }}
                ...
                <div class="form-group">
                    <a href="/tagihan-blbi/{{ $tagihan->id }}/rekanan/{{ $rekanan->id }}/ppn" class="btn btn-xs btn-accent">Batal</a>
