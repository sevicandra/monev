...
@section('content')
    <div class="bg-primary p-4">
        <h1 class="text-xl text-primary-content">Tambah Tagihan BLBI</h1>
    </div>
    <div class="px-4 gap-2 overflow-y-auto">
        <form action="/tagihan-blbi" method="post" autocomplete="off">
            @csrf
            ...
            <div class="form-control w-full max-w-xs">
                ...
                <select type="text" name="ppk"
                    ...
                    @foreach ($ppk as $item)
                        <option value="{{ $item->nip }}" @if (old('ppk') === $item->nip) selected @endif>
                            {{ $item->nama }}</option>
                    @endforeach
                </select>
                <label class="label">
