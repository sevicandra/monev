...
@section('content')
    <main class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Realisasi PPK {{ $ppk->nama }}</h1>
        </div>
        <div class="row mb-3">
            <div class="col-lg-7">
                <a href="/dashboard/ppk/{{ $ppk->id }}" class="btn btn-sm btn-outline-primary mt-1 @if (request('sp2d') === 'ya') @else active @endif">Per Tagihan</a>
                <a href="/dashboard/ppk/{{ $ppk->id }}?sp2d=ya" class="btn btn-sm btn-outline-primary mt-1 ml-2 @if (request('sp2d') === 'ya') active @else @endif">Per SP2D</a>
            </div>
            <div class="col-lg-5">
            </div>
        </div>
                            ...
                            @foreach ($data as $item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @else
                                        <a href="/dashboard/ppk/{{ $ppk->id }}/null/tagihan/?sp2d={{ request('sp2d') }}" target="_blank">
                                            Rp {{ number_format($item->total_realisasi, 2, ",", ".") }}
                                        </a>  
                                        @endif
                                    </td>
