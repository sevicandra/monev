...
@section('content')
    <div class="bg-primary p-4">
        <h1 class="text-xl text-primary-content">Tambah Pagu PPk {{ $ppk->nama }}</h1>
    </div>
    <div class="">
    ...
    <div class="flex flex-col md:flex-row px-4 gap-2 justify-between">
        <div class="lg:basis-5/12 flex gap-2">
            <a href="/maping-ppk/{{ $ppk->id }}/pagu" class="btn btn-sm btn-neutral"> Kembali</a>
        </div>
        ...
        <x-coa :realisasi="false" :sisa="false" :pengembalian="false" :unit="true">
            @foreach ($data as $item)
                <tr>
                    ...
                    <x-table.body.column
                        class="border text-center">{{ $item->program }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-center">{{ $item->kegiatan }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-center">{{ $item->kro }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-center">{{ $item->ro }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-center">{{ $item->komponen }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-center">{{ $item->subkomponen }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-center">{{ $item->akun }}</x-table.body.column>
                    <x-table.body.column
                        class="border text-right">Rp{{ number_format($item->anggaran, 2, ',', '.') }}
                    </x-table.body.column>
                    ...
                    <x-table.body.column class="border text-center">
                        <div class="join">
                            <form action="/maping-ppk/{{ $ppk->id }}/pagu/{{ $item->id }}" method="post">
                                @csrf
