...
<main class="col-md-9 ml-sm-auto col-lg-10 px-md-4">
    ...
    <div class="row mb-3">
        <div class="col-lg-7">
            <a href="/tagihan/{{ $tagihan->id }}/payroll" class="btn btn-sm btn-outline-secondary mt-1 mb-1 ml-2">Sebelumnya</a>
        </div>
        <div class="col-lg-5">
                    ...
                    <tbody>
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td id="nama_{{ $item->id }}">{{  $item->nama  }}</td>
                                <td id="norek_{{ $item->id }}">{{  $item->norek  }}</td>
                                <td id="bank_{{ $item->id }}">@if ($item->bank === 'Other'){{$item->otherbank}}@else{{$item->bank}}@endif</td>
                                <td class="pb-0">
                                    <div class="btn-group btn-group-sm" role="group">
                                        <button id="{{ $item->id }}" type="button" class="import-btn btn btn-sm btn-outline-secondary pt-0 pb-0">Pilih</button>
                                    </div>
                                </td>
                            </tr>
                         @endforeach
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-body">
            <form action="http://monev.test/tagihan/{{ $tagihan->id }}/payroll/import" method="post" autocomplete="off">
                @csrf
                <div class="row">
                    <div class="col-lg-12">
