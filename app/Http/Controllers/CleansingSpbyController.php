...
class CleansingSpbyController extends Controller
{
    ...
    public function download()
    {
        ...
        $spreadsheet->setActiveSheetIndex(1)
            ->setCellValue('B3', 'Daftar SPBy Belum Input SP2D')
            ->mergeCells('B3:I3')
        ...
        header('Content-Disposition: attachment;filename="SPBy -' . date('D, d M Y H:i:s') . '.xlsx"');
    ...
    public function store(Request $request)
    {
        ...
        if ($errors->min('status') === TRUE) {
            return redirect('/cleansing/spby')->with('berhasil', 'Data Berhasil Diuplaod');
        } else {
            return redirect('/cleansing/spby/import')->with('pesan', $errors->sum('status') . ' Data Berhasil Diupload Silakan Download Kembali Data SPBy')->with('rowsErrors', collect($errors));
        }
    }
}
